...
class TradingRobot:
    def __init__(self, symbols, lot_size=0.5, stop_loss=50, take_profit=10):
    ...
    def get_signal(self, symbol):
        # Mengambil data historis dengan timeframe M15
        ...
        if (df['MA5'].iloc[-1] > df['MA10'].iloc[-1] and 
            ...
            df['close'].iloc[-1] > df['MA50'].iloc[-1]):  # Konfirmasi tambahan
            return "BUY"
...
def main():
    ...
    robot = TradingRobot(
        symbols=["EURUSD", "GBPJPY","AUDUSD", "USDJPY", "XAUUSD", "BTCUSD","USDCAD"],
        lot_size=0.02,  # Ukuran lot yang ditetapkan menjadi 0.02
    ...
    while True:
        try:
            ...
            for symbol in robot.symbols:
                ...
                if total_entries < 10:  # Hanya lakukan entry jika total entry kurang dari 10
